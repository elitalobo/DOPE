...
class Plotter():
    def __init__(self, path, dir="plots1/",axis=0):
        self.path = path
        self.axis=axis
        ...
        if os.path.exists("plots1/experiments10") == False:
            # os.rmdir("plots/experiments5")
            os.mkdir("plots1/experiments10")

        if os.path.exists("plots1/experiments11") == False:
            # os.rmdir("plots/experiments5")
            os.mkdir("plots1/experiments11")

        if os.path.exists("plots1/experiments12") == False:
            # os.rmdir("plots/experiments5")
            os.mkdir("plots1/experiments12")

        if os.path.exists("plots1/experiments13") == False:
            # os.rmdir("plots/experiments5")
            os.mkdir("plots1/experiments13")
    ...
    def experiment1(self):
        ...
        for file in os.listdir(self.path +"/"):
            try:
                ...
                if dataset_id <=0 and dataset_id > 10:
                    continue
                ...
                if "_policy1" in file:
                    if "IS_is-mountaincar-influence"==key:
                        print("here")
                    plt.clf()
                    ...
                    if res.get(key) is None:
                        res[key] = []
                    rets1 = np.abs(100*(rets - init_ret))/(np.abs(init_ret)+ 1e-10)

                    res[key].append(rets1)
        ...
        for key, value in res.items():
            try:
                ...
                self.plot("experiments1/" + key + "_experiment1",value1,self.projections[:2],self.budget_range,xlabel,ylabel,std_lower,std_upper)
            except:
                print("failedhere")
                print(key)
                pass
    ...
    def experiment10(self, res_new):
        ...
        for file in os.listdir(self.path +"/"):
            try:
                ...
                if dataset_id <=0 and dataset_id > 10:
                    continue
                if dataset_id in np.arange(20,25):
                    continue
                method = name[1]
                if (name[1]=="IS" or name[1]=="WDR"):
                    method = name[1] + "_" + name[2]
        ...
        for key, value in res.items():
            try:
                ...
                value1 = self.func(value,0)[self.axis,:].reshape(1,-1)
                std1_lower, std1_upper = get_ci_intervals(value)
                std1_lower = std1_lower[self.axis,:]
                std1_upper = std1_upper[self.axis,:]
    ...
    def experiment11(self, res_new):
        ...
        for key, value in res.items():
            try:
                ...
                self.plot("experiments11/" + key + "experiment11",final_val,["FSGM Attack","DOPE Attack"],self.budget_range,xlabel,ylabel,final_std_lower,final_std_upper,add=False)
            except:
                pass
        # for key, value in res_train.items():
        #     value = self.func(value,0)
    ...
    def experiment5_(self,res_new, res_random,res_fsgm):
        res={}
        res_train={}
        for file in os.listdir(self.path +"/"):
            try:
        ...
        for key, value in res.items():
            ...
            try:
                ...
                rs = res_random[key]
                value3 = self.func(rs,0)[self.axis,:].reshape(1,-1)
                # std3 = np.std(rs,0)[0,:].reshape(1,-1)
                std3_lower, std3_upper = get_ci_intervals(rs)
                std3_lower = std3_lower[self.axis,:]
                std3_upper = std3_upper[self.axis,:]


                rs_fsgm = res_fsgm[key]
                value4 = self.func(rs_fsgm,0)[self.axis,:].reshape(1,-1)
                # std3 = np.std(rs,0)[0,:].reshape(1,-1)
                std4_lower, std4_upper = get_ci_intervals(rs_fsgm)
                std4_lower = std4_lower[self.axis,:]
                std4_upper = std4_upper[self.axis,:]

                final_val = np.stack((value2,value1, value3, value4),axis=0).squeeze()
                final_std_lower = np.stack((std2_lower,std1_lower, std3_lower, std4_lower),axis=0).squeeze()
                final_std_upper = np.stack((std2_upper,std1_upper, std3_upper, std4_upper),axis=0).squeeze()
                ...
                res_all[env]["FSGM-based Attack"]["mean"]=value4
                res_all[env]["FSGM-based Attack"]["std_lower"]=std4_lower
                res_all[env]["FSGM-based Attack"]["std_upper"]=std4_upper
            ...
            self.plot("experiments12/" + key + "experiment12",final_val,["DOPE","Random DOPE","Random Attack", "FSGM-based Attack"],self.budget_range,xlabel,ylabel,final_std_lower, final_std_upper,add=False)
            # except:
            #     pass
            # except:
            #     pass
    ...
    def plot_transitions(self,path):
        ...
        fig.savefig("plots/custom_example.pdf")
    ...
    def experiment6(self,flag=False):
        ...
        for key, value in res.items():
            ...
            plt.savefig("plots/experiments6/"  + key + "_before_wts.pdf")
            plt.clf()
            sns.displot(aw,  kind="kde", fill=True, color=self.colors[1])
            plt.savefig("plots/experiments6/"  + key + "_after_wts.pdf")
            plt.clf()
    ...
    def experiment8(self, res, flag=False):
        ...
        for key, value in res.items():
            ...
            if results[env].get(method) is None:
                results[env][method]={}
            results[env][method]["mean"] = self.func(value,0)[self.axis,:]
            # results[env][method]["std"] = np.std(value,0)[0,:]
            results[env][method]["std_lower"], results[env][method]["std_upper"] = get_ci_intervals(value)
            results[env][method]["std_lower"]=  results[env][method]["std_lower"][self.axis,:]
            results[env][method]["std_upper"]=  results[env][method]["std_upper"][self.axis,:]
        ...
        for key, value in results.items():
            ...
            try:
                ...
                ylabel="Percentage error in value function"
                self.plot("experiments8/" + key + "experiment88" + label,values_final,methods2,xvals,xlabel,ylabel,stds_final_lower,stds_final_upper,add=add)
                print("experiment 8")
                plt.clf()
                print(key)
                print(methods)
                ...
                stds_final_upper = np.stack(stds_upper,axis=0)
                ylabel="Percentage error in value function"
                self.plot("experiments8/" + key + "subset_experiment88" + label,values_final,methods2,xvals,xlabel,ylabel,stds_final_lower,stds_final_upper,add=add)
                plt.clf()
                ...
                print(methods)
                for idx in range(len(methods)):
                    print("2nd" + methods2[idx] + "," + str(np.round(values_final[idx,-1],2)) + "," + str(np.round(stds_final_lower[idx,-1],2)) + ","+ str(np.round(stds_final_upper[idx,-1],2)))
    ...
    def experiment9(self,paths):
        ...
        for path in paths:
            ...
            plt.savefig("plots/experiments9/"+ path + "_original.pdf")
            plt.clf()
            ...
            ax.set_aspect('equal')
            ax.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.0)
            plt.savefig("plots/experiments9/"+ path + "_corrupt.pdf")
    ...
    def experiment12(self, res_new):
        ...
        for file in os.listdir(self.path +"/"):
            try:
                ...
                if "FQE" in file and "cancer" in file:
                    print("found")
        ...
        for key, value in res.items():
            try:
                ...
                if "FQE" in key:
                    print("found fqe")
                self.plot("experiments13/" + key + "experiment13",final_val,["Projected DOPE Attack","DOPE Attack"],self.budget_range,xlabel,ylabel,final_std_lower,final_std_upper,add=False)
            except:
                pass
        # for key, value in res_train.items():
        #     value = self.func(value,0)
...
def get_results(res,res10, plotter):
    domains=['custom', "cancer", "hiv"]
    methods=["FQE","IS_is","IS_pdis","IS_cpdis","WDR_dr","WDR_wdr"]
    for domain in domains:
        values = []
        values_25 = []
        for method in methods:
            ...
            values.append(val)
            values_25.append(plotter.func(value,0)[0,5])
        ...
        for method in methods:
            ...
            val = plotter.func(value,0)[0,-1]
            values1.append(val)
            values1_25.append(plotter.func(value,0)[0,5])
...
r10 = plotter.experiment10(res)
r11 = plotter.experiment12(res)
# get_results(res,r10,plotter)
res_2=plotter.experiment2()
# # plotter.experiment3()
res_r=plotter.experiment4(res)
# r5=plotter.experiment5(res,res_r)
plotter.experiment5(res,res_r)
r5=plotter.experiment5_(res,res_r,r10)

# r2=plotter.experiment8(res,flag=True)
r3=plotter.experiment8(res_2,flag=False)
