...
def experiment1(env,eps=0.05, trials=1):
    budgets =np.arange(0.0,2.0,0.3)

    init_returns_policy1 = []

    projections = ["linf","l1","l2"]
    init_sum=[]
    current_budgets = []
    attacker=None
    index=0
        ...
        for budget in budgets:
            ...
            for trial in range(trials):
                ...
                attacker = InfluenceAttack(method_type=args.method_type, env=env, config=config, sign=args.sign, initial_lr=args.initial_lr,
                                           type=projection, epsilon=args.epsilon,  max_epochs=args.max_epochs, reg=args.reg,eps=eps, random=0, iters=args.iters, attacker_type=args.attacker_type,is_type=args.is_type,dataloader=None,frac=budget, dataset_id=args.dataset_id)
                ...
                if (projection=="linf" and  args.method_type=="IS"):
                    try:
...
if __name__ == '__main__':
    ...
    # experiment4(env,trials=100)
        ...
        experiment4(env,trials=2)


        experiment6(env,trials=2)

        experiment5(env,trials=2)
