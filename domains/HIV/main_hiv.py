...
sys.path.append("src/")
sys.path.append("../src/")
sys.path.append("hiv_domain/")

from memory import *
from utils_hiv import *
...
def generate_data(env, config, eval_env=None):
    ...
    if config.standardize_rewards:
        ...
        #with open("domains/HIV/hiv_domain/standardization_data.pkl", "rb") as fobj:
    ...
    for i_episode in range(config.sample_num_traj):
        ...
        while not done:
            ...
            p_pie = torch.FloatTensor([p_pie])
            p_pib = torch.FloatTensor([p_pib])
...
def generate_trajectories(env, config, eval_env=None):
    t0 = time()
    memory = SampleSet(config)
    dev_memory = SampleSet(config)
    traj_set = TrajectorySet(config)
    ...
    for i_episode in range(config.sample_num_traj):
        ...
        while not done:
            ...
            traj_set.push(state, action, next_state, reward, done, isweight, None,
                          n_steps, factual, last_factual, None, None, None, None, None)
            trajectory.append(Transition(state, action, next_state, reward, done, isweight))
            n_steps += 1
    ...
    print("Generating {} trajectories took {} minutes".format(
        config.sample_num_traj, (t1 - t0) / 60))
    return memory, dev_memory, trajectories, scores
...
#     env.tree = joblib.load('hiv_domain/hiv_domain_extra_tree_gamma_ins20.pkl')
#
...
#                            ins=config.ins,
#                            episode_length=config.max_length)
#     eval_env.tree= joblib.load('hiv_domain/hiv_domain_extra_tree_gamma_ins20.pkl')
#
#     if config.fix_data:
...
#
#
#         print("HIV MDP model", np.mean(estm_rescaled))
