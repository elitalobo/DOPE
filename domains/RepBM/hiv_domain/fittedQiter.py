    ...
    def policy(self, state,eps = 0.15):
        ...
        if len(state.shape)==1 or state.shape[0]==1:
            if np.random.rand(1) < eps:
                return np.random.randint(0, self.num_actions, state.shape[0])
            else:
                return self.tree.predict([np.hstack([state.flatten(), a]) for a in range(self.num_actions)]).argmax()
        else:
            action_arr = (np.ones((state.shape[0],self.num_actions))*np.arange(self.num_actions)).reshape(-1,1)
            state_actions = np.concatenate((np.repeat(state,axis=0,repeats=4),action_arr),axis=1)
            return self.tree.predict(state_actions).reshape(-1,self.num_actions).argmax(axis=1)
