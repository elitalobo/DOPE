...
#transition noise =0.03
#transition noise for gamma 0.003
#TODO change transition noise and noise
class CancerEnv:
    def __init__(self, dose_penalty=0.1, max_steps=30, transition_noise=0.0,discount_factor=0.95):
        ''' Intitilize patient parameters '''
        self.kde = 0.24
        self.name = "cancer_domain"
        self.action_dim = 2
        self.state_dim = 4
        self.lambda_p = 0.121
        self.k_qpp = 0.0031
    ...
    def perform_action(self, action):
        ...
        noise = 1 + self.transition_noise * np.random.gamma(4)
        next_state *= noise
    ...
    def step(self,action):
        next_state, reward = self.perform_action(action)
        done = self.is_done()
