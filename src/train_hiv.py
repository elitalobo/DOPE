...
sys.path.append("src/")
from fitted_Q_learning import FittedQIteration
...
class DeepFittedQIteration():
    ...
    def __init__(self, env, iterations = 400, K = 10, num_patients = 30, preset_params = None, ins = None, \
                 perturb_rate = 0.0, episode_length = 200, cache=False,config=None, tree=None):
    ...
    def run_episode(self, eps = 0.05, track = False,debug=False, select_max=False, guide_action=False):
        ...
        while ((not done) and ( n_steps < self.config.max_length)):
            ...
            # action1 = self.tree.policy(np.zeros(6), eps=0.0)[0]

            # if isinstance(action,np.ndarray)==False:
    ...
    def guided_action(self,state):
        ...
        action = self.tree.policy(state_np, eps=0.0)[0]
        action = torch.from_numpy(np.array([action])).reshape(-1,1)
        # action = torch.ones(1,1).long()
        # action = torch.tensor(action_arr).long()
        ...
        if len(state.shape)==1 or state.shape[0]==1:
            if np.random.rand(1) < eps:
                ...
                probs = softmax(self.qnet(
                    state)+1e-10).reshape(self.env.action_dim)*(1-eps) + eps/self.num_actions
                return self.qnet(
                    state.type(DoubleTensor)).max(1)[1].view(1, 1), probs
        else:
            probs = softmax(self.qnet(
    ...
    def updatePlan(self,num_episodes=None):
        ...
        for i_episode in range(num_episodes):
            ...
            ep_list, reward = self.run_episode(eps=0.0,guide_action=False) #TODO : Set to false
...
if __name__ == '__main__':
    ...
    for num_episodes in num_eps:
        ...
        #config = hiv_config
        env = HIVEnv()
        qtree = FittedQIteration(env=env,ins=20)

        qtree.tree = joblib.load('policies/hiv_domain_extra_tree_gamma_ins20_K9.pkl')

        qiter = DeepFittedQIteration(env=env,ins=20,config=config,tree=qtree)
