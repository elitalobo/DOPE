...
from models import QtNet, MDPnet, TerminalClassifier
from memory import MRDRTransition, TrajectorySet, SampleSet
from utils import *
...
def mrdr_loss(q_values, onehot_rewards, weights, pib, pie):
    ...
    Omega = DoubleTensor(batch_size, action_size, action_size).zero_()
...
def mrdr_train(samples, qnet, optimizer, loss_mode, config, permutation, i_batch, wis_reward=False):
    ...
    onehot_rewards = DoubleTensor(num_samples,config.action_size).zero_() # batch_size x action_size
...
def mdpmodel_train(memory, mdpnet, optimizer, loss_mode, config):
    ...
    weights = DoubleTensor(config.train_batch_size)
    weights.fill_(1.0)
...
def mdpmodel_test(memory, mdpnet, loss_mode, config):
    ...
    factual = torch.cat([t.factual for t in transitions])
    weights = DoubleTensor(config.test_batch_size)
    weights.fill_(1.0)
    weights = weights + factual * ratio
    weights_2 = factual * ratio
...
def terminal_classifier_train(memory, model, optimizer, batch_size):
    ...
    done = torch.cat([DoubleTensor([[t.done and t.time < 199]]) for t in transitions])
    ...
    acc = ((predict_soft_done.detach()[:,0]>0).Double()==all_done[:,0]).sum().item()/(all_done.size()[0])
...
def rollout_batch(init_states, mdpnet, is_done, num_rollout, policy_qnet, epsilon, action_size, maxlength, config, init_done=None, init_actions=None):
    ...
    while not (sum(done.long() == batch_size) or n_steps >= maxlength):
        if n_steps > 0:
            actions = epsilon_greedy_action_batch(states, policy_qnet, epsilon, action_size)
        states_re = Tensor(np.double(config.rescale)*states)
        ...
        states_diff = 1 / np.double(config.rescale) * states_diff
        next_states = states_diff + states
        states = next_states
        t_reward = t_reward + (1 - done).Double() * reward
...
def compute_values(traj_set, model, is_done, policy_qnet, config, model_type='MDP', soften=False):
    ...
    state_tensor = DoubleTensor(num_samples, config.max_length, config.state_dim).zero_()
    action_tensor = LongTensor(num_samples, config.max_length, 1).zero_()
    pie_tensor = DoubleTensor(num_samples, config.max_length, config.action_size).zero_()
...
def train_pipeline(env, config, eval_qnet, seedvec = None):
    ...
    for i_episode in range(config.sample_num_traj):
        ...
        n_steps = 0
        acc_soft_isweight = DoubleTensor([1])
        ...
        while not done:
            ...
            next_state_re = np.double(config.rescale)*next_state
            state_re = np.double(config.rescale)*state
